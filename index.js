...
client.on('messageCreate', async (message) => {
  if(message.content.startsWith(prefix + "rconM") && message.member.roles.cache.some(role => role.name === process.env.ROLE)){
    message.delete(1000);
      ...
      await sleep(2000)
      if(rconRes == ""){rconRes =  "'" + rconString + "' message has been successfully sent to the server!"} 
  ...
  if(message.content.startsWith(prefix + "rconA") && message.member.roles.cache.some(role => role.name === process.env.ROLE)){
    ...
    await sleep(2000)
    if(rconRes == ""){rconRes = "Server will restart in " + rconString + " minute, announcement was successfully sent to the server!"} 
    message.channel.send(rconRes)
    rconRes = ""
    conn.disconnect();
}
